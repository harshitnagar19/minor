...
const WordToPdfConverter = () => {
  ...
  const handleConvert = async () => {
    if (!selectedFile) {
      setErrorMessage('Please select a Word document to convert');
      return;
    }
    ...
    try {
      ...
      // Generate PDF filename from the original filename
      const originalFileName = selectedFile.name;
      const pdfFileName = originalFileName.substring(0, originalFileName.lastIndexOf('.')) + '.pdf';
  ...
  return (
    <div className="bg-gray-900 text-white flex justify-center pt-20">
      <div className="max-w-md mx-auto my-8 p-6 bg-gray-800 text-white rounded-lg shadow-md">
        <h1 className="text-2xl font-bold mb-6 text-center text-indigo-400">Word to PDF Converter</h1>
        ...
        {conversionComplete && (
          <div className="mt-8 border-t border-gray-600 pt-4">
            <div className="bg-green-900/30 border border-green-700 rounded-md p-4 text-center">
              ...
              <p className="text-green-300 text-sm mt-1">Your PDF has been downloaded</p>
            </div>
          </div>
        )}
      </div>
